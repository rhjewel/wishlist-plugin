...
<article class="egwl-item" data-post-id="<?php echo esc_attr( $post_id ); ?>" role="row">
	<div class="egwl-remove-cell" role="cell">
		<?php if ( $settings->get_bool( 'show_remove_button' ) ) : ?>
			<button type="button" class="egwl-remove-button" data-post-id="<?php echo esc_attr( $post_id ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Remove %s from wishlist', 'wishflow' ), get_the_title( $post_id ) ) ); ?>">&times;</button>
		<?php endif; ?>
	</div>
	...
	<div class="egwl-product-price" role="cell" data-label="<?php esc_attr_e( 'Unit Price', 'wishflow' ); ?>">
		<?php echo $price_html ? wp_kses_post( $price_html ) : '&mdash;'; ?>
	</div>

	<div class="egwl-item-actions" role="cell" data-label="<?php esc_attr_e( 'Actions', 'wishflow' ); ?>">
		...
		<?php if ( $settings->get_bool( 'show_view_button' ) ) : ?>
			<a class="button egwl-view-button" href="<?php echo esc_url( get_permalink( $post_id ) ); ?>"><?php esc_html_e( 'View', 'wishflow' ); ?></a>
		<?php endif; ?>
	</div>
</article>
